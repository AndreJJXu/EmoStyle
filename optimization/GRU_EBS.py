...
def Embedding_lyrics(lyric, syllModel, wordModel, lyc2vec):
...
def compute_emotion_score(softmax, syllable_vocabulary, device, classifier, emotion, EBS_sets, candidate_pairs, b3):
    new_EBS_sets = []
    if not classifier:
...
def emotional_beam_search(softmax, syllable_vocabulary, device, syllModel, wordModel, encoder, Ge_lyric_vocabulary, music_index2note, lyc2vec, args, seed_lyric, generator, classifier, outlen, b1, b2, b3):
    ...
    for i in range(seed_len-1):
        ...
        lyric_input = Embedding_lyrics(lyric_input, syllModel, wordModel, lyc2vec)
    ...
    while True:
        ...
        EBS_sets = compute_emotion_score(softmax, syllable_vocabulary, device, classifier, args.emotion, EBS_sets, candidate_pairs, b3)
    ...
    for i in range(args.output_num):
        ...
        # print('------------')
        # print(midi_pattern.instruments)
        # print('--------------')
    return lyric_i
...
# fs.midi_to_audio('negative0.mid', 'output3.wav')