...
def gru_ebs(args, wav_path):
    ...
    syll_model_path = '/Generate-Lyrics-and-Melody-with-Emotions/Skip-gram_lyric_encoders/syllEncoding_skipgram_dim_'+str(args.lyc2vec)+'.bin'
    word_model_path = '/Generate-Lyrics-and-Melody-with-Emotions/Skip-gram_lyric_encoders/wordLevelEncoder_skipgram_dim_'+str(args.lyc2vec)+'.bin'
    ...
    generator_file = '/Generate-Lyrics-and-Melody-with-Emotions/GRU_generator_seqlen_50_embed_128_epoch_59.pkl'
    binary_clf_file = '/Generate-Lyrics-and-Melody-with-Emotions/saved_model/'+'LSTM_datasetlen_'+str(seqlen)+'_fold_7_clf.pkl'

    word_vocabulary_file = '/Generate-Lyrics-and-Melody-with-Emotions/saved_model/word_vocabulary.npy'
    word_vocabulary = np.load(word_vocabulary_file)
    word_vocabulary = word_vocabulary.item()
        
    syllable_vocabulary_file = '/Generate-Lyrics-and-Melody-with-Emotions/saved_model/syllable_vocabulary.npy'
    syllable_vocabulary = np.load(syllable_vocabulary_file)
    syllable_vocabulary = syllable_vocabulary.item()
        
    music_vocabulary_file = '/Generate-Lyrics-and-Melody-with-Emotions/saved_model/music_vocabulary_'+str(seqlen)+'.npy'
    ...
    seed_lyric = [[word] for word in split_words]
    print("seed_lyric:", seed_lyric)

    emotional_beam_search(softmax, syllable_vocabulary, device, syllModel, wordModel, encoder, Ge_lyric_vocabulary, music_index2note, lyc2vec, args, seed_lyric, generator, classifier, outlen = args.outlen, b1=args.b1, b2=args.b2, b3=args.b3)
    fs=FluidSynth('/Generate-Lyrics-and-Melody-with-Emotions/MuseScore_General.sf2')
    fs.midi_to_audio('positive0.mid', wav_path)
...
def chatglm_deploy(args):
    ...
    model = model.eval()
    print("Ready!")
    Prompt = "classify sentiment polarity (Positive or Negative):"
    input_str = Prompt + args.description
...
def main(args):
    ...
    #chatGLM-based Sentiment classification
    args.emotion = chatglm_deploy(args)

    #preprocess audio input
    args.audio_extra = args.results_dir + "/tmp.wav"
    ...
    torch.save(latent, args.results_dir + "/latent_code.pt")
    print("latent:", latent)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--audio_path", type=str, default="/sound-guided-semantic-image-manipulation/data/test.wav")
    ...
    parser.add_argument("--audio_extra", type=str, default="/Sound-Image-Generation/sound-guided-semantic-image-manipulation/tmp.wav")
